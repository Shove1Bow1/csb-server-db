...
async function findAllReports(phoneNumber, code) {
  try {
    const result = await phoneNumbersSchema
      .find({
        where: {
...
async function createReport({
  ...
  if (existNumber[0]) {
    ...
    throw {
      message: "This device already reported number - conflict happen",
      status: "409",
    };
  }
...
async function updateStatus(existNumber) {
  ...
  if (
    (await numberOfReports) >= POTENTIAL_SCAMMER &&
    (await numberOfReports) < SCAMMER
  ) {
...
async function suggestSearchingWithSpammer(phoneNumber) {
